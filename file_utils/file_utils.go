...
// ReadFromFile 从文件中读取字节数据
func ReadFromFile(src string) ([]byte, error) {
	// 打开文件
	file, err := os.Open(src)
	if err != nil {
		return nil, err // 返回文件打开失败的错误
	}
	...
	if err != nil {
		return nil, err // 返回文件读取失败的错误
	}

	return buffer, nil // 成功则返回读取的数据
}
